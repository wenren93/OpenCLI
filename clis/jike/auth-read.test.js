...
describe('Jike identity guard', () => {
  ...
  it('accumulates and deduplicates feed posts across virtualized snapshots', async () => {
    const first = { id: 'post-1', content: 'first' };
    const second = { id: 'post-2', content: 'second' };
    const third = { id: 'post-3', content: 'third' };
    const page = makePage(identity, [first, second], undefined, [second, third]);

    await expect(getRegistry().get('jike/feed').func(page, { limit: 3 }))
      .resolves.toEqual([first, second, third]);
    expect(page.evaluate).toHaveBeenCalledTimes(4);
    expect(page.evaluate.mock.calls[2][0]).toContain('.mantine-ScrollArea-viewport');
  });

  it('tolerates temporary stale feed snapshots and stops after five consecutive stale scrolls', async () => {
    ...
    const page = makePage(
      ...
      undefined, [first],
      undefined, [second],
      ...Array.from({ length: 5 }, () => [undefined, [second]]).flat(),
    );

    await expect(getRegistry().get('jike/feed').func(page, { limit: 20 }))
      .resolves.toEqual([first, second]);
    expect(page.evaluate).toHaveBeenCalledTimes(16);
  });
